...
import static me.vovanov.homgund.Social.Ignore.IgnoreImpl.*;
...
public class UnignoreCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (ignorerName.equals(ignoredPlayerName)) {
            sender.sendMessage(text("Нельзя перестать игнорировать самого себя", RED));
            return true;
        }
        ...
        if (!list.contains(ignoredUUID.toString())) {
            player.sendMessage(text("Вы не игнорируете этого игрока", RED));
            return true;
        }
        unignorePlayer(ignorerUUID, ignoredUUID.toString());
        player.sendMessage(text("Вы более не игнорируете сообщения "+ignoredPlayerName, GREEN));
        return true;
    }
